...
int main(void)
{
    ...
    float ramp_target[MOTOR_COUNT];
    bool  ramping[MOTOR_COUNT];
    ...
    LOGI("Init OK. Cmd: pid[1-4] kp=X ki=X kd=X ff=X tgt=X ilim=X sep=X flt=X");
    LOGI("Default: P=%.2f I=%.2f D=%.2f FF=%.2f tgt=%.1fmm/s ilim=%.0f sep=%.1f flt=%.2f",
    ...
    while (1)
    {
        if (g_usart_rx_sta & 0x8000)
        {
            ...
            if (cmd_parse(g_usart_rx_buf, rx_len, &cmd))
            {
                ...
                if (cmd.motor_id > 0) {
                    ...
                    LOGI("Cmd received for motor %d, stopping...", cmd.motor_id);
                } else {
                    LOGI("Cmd received, stopping all motors...");
                }
                ...
                last_tick = HAL_GetTick();
                LOGI("Motors restarted.");
            }
            else
            {
                LOGI("Invalid cmd: %s", g_usart_rx_buf);
            }
        ...
        if (HAL_GetTick() - last_tick >= PID_PERIOD_MS)
        {
            ...
            for (int i = 0; i < MOTOR_COUNT; i++) {
                if (ramping[i])
                {
                    ramp_target[i] += RAMP_RATE_MM_S2 * PID_PERIOD_S;
                    if (ramp_target[i] >= tgt_speed_mm_s[i])
                    {
                        ramp_target[i] = tgt_speed_mm_s[i];
                        ramping[i] = false;
                    }
                }
                float setpoint = ramping[i] ? ramp_target[i] : tgt_speed_mm_s[i];
                ...
                if (pid_out > (float)MOTOR_SPEED_MAX) pid_out = (float)MOTOR_SPEED_MAX;
                ...
                if (g_usart_rx_sta & 0x8000)
                {
                    uint16_t rx_len2 = g_usart_rx_sta & 0x3FFF;
                    g_usart_rx_buf[rx_len2] = '\0';

                    if (cmd_parse(g_usart_rx_buf, rx_len2, &cmd))
                    {
                        int s = 0, e = MOTOR_COUNT;
                        if (cmd.motor_id > 0) {
                            s = (int)s_motor_map[cmd.motor_id];
                            e = s + 1;
                        }
                        for (int j = s; j < e; j++) {
                            motor_set_speed((motor_id_t)j, 0);
                            speed_filtered[j] = 0.0f;
                        }
                        apply_params(pid, &filter_alpha, &cmd, tgt_speed_mm_s);
                        for (int j = s; j < e; j++) {
                            pid_reset(&pid[j]);
                            motor_reset_encoder((motor_id_t)j);
                            ramp_target[j] = 0.0f;
                            ramping[j] = true;
                        }
                        delay_ms(3000);
                        for (int j = s; j < e; j++) {
                            last_enc[j] = motor_get_encoder((motor_id_t)j);
                        }
                        last_tick = HAL_GetTick();
                        LOGI("Motors restarted.");
                    }
